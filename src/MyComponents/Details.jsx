import React, { Component } from "react";
import "./Details.css";
import Dcontent from "./Dcontent";
import img from "../img/profile.jpg";

function Details() {
  let link =
    "data:image/jpeg;base64,";

  let data = [
    {
      source: link,
      title: "Rajan Maharjan",
    },
    {
      source: "https://static.xx.fbcdn.net/rsrc.php/v3/y8/r/S0U5ECzYUSu.png",
      title: "Friends",
    },

    {
      source: "https://static.xx.fbcdn.net/rsrc.php/v3/y5/r/PrjLkDYpYbH.png",
      title: "Groups",
    },
    {
      source: "https://static.xx.fbcdn.net/rsrc.php/v3/yU/r/D2y-jJ2C_hO.png",
      title: "Marketplace",
    },
    {
      source: "https://static.xx.fbcdn.net/rsrc.php/v3/y5/r/duk32h44Y31.png",
      title: "Watch",
    },
    {
      source: "https://static.xx.fbcdn.net/rsrc.php/v3/y8/r/he-BkogidIc.png",
      title: "Memories",
    },
    {
      source: "https://static.xx.fbcdn.net/rsrc.php/v3/yD/r/lVijPkTeN-r.png",
      title: "Saved",
    },
    {
      source: "https://static.xx.fbcdn.net/rsrc.php/v3/yH/r/kyCAf2jbZvF.png",
      title: "Pages",
    },

    {
      source: "https://static.xx.fbcdn.net/rsrc.php/v3/yc/r/hTN47HVa4oS.png",
      title: "Most Recent",
    },
    {
      source:
        "https://www.pngmart.com/files/3/Down-Arrow-PNG-Free-Download.png",
      title: "See More",
    },
  ];

  return (
    <React.Fragment>
      <div className="details">
        <div className="detailsContent">
          {data.map((data) => {
            return <Dcontent source={data.source} title={data.title} />;
          })}
          <hr />
          <h6>Your Shortcuts</h6>
          <Dcontent
            source="https://scontent.fktm10-1.fna.fbcdn.net/v/t39.2081-6/10574700_1002395243110827_234271886_n.png?stp=c6.6.31.31a_dst-png_p36x36&_nc_cat=111&ccb=1-7&_nc_sid=eaa83b&_nc_ohc=rnm0NnZDAmAAX8TzEOn&_nc_ht=scontent.fktm10-1.fna&oh=00_AT-CWN4XQ8iIUAf4czWycuofFdA_9QKFZq75Lu9__Nu-vg&oe=62B801F6"
            title="Teen Patti Gold"
          />
          <div
            className="footer"
            style={{
              display: "flex",
            }}
          >
            <a href="#">Privacy </a> .<a href="#">Terms </a> .
            <a href="#">Advertising </a> .<a href="#"> Ad choices</a>
          </div>

          <div
            className="footer"
            style={{
              display: "flex",
            }}
          >
            <a href="#">
              <i
                style={{ fontSize: "13px" }}
                class="fa-solid fa-hand-holding-hand"
              ></i>
            </a>{" "}
            .<a href="#">Cookies </a> .<a href="#">More </a> .
            <a href="#"> Meta@2022</a>
          </div>
          <br />
        </div>
      </div>
    </React.Fragment>
  );
}

export default Details;
